...
def compute_rotation_params(a_pp: float, a_qq: float, a_pq: float) -> tuple[float, float]:
    """
...
def jacobi_rotation_method(
    ...
    while iterations < max_iterations:
        ...
        # Обновление элементов матрицы A вручную
        for i in range(n):
            if i != p and i != q:
                aip = A[i][p]
                aiq = A[i][q]
...
def qr_decomposition(A: Matrix) -> tuple[Matrix, Matrix]:
    ...
    for j in range(m):
        ...
        if norm == 0:
            raise ValueError("Невозможно построить ортонормальный базис (линейно зависимые векторы).")
    ...
    # Построим Q как матрицу из колонок q_0, q_1, ...
    Q = Matrix(n, m)
    for j in range(m):
        for i in range(n):
...
def qr_algorithm(A: Matrix, eps: float = 1e-9, max_iterations: int = 1000) -> list[float]:
    """
    ...
    for iteration in range(max_iterations):
        ...
        # Оценка сходимости: по изменениям диагонали
        max_delta = max(
            abs(A_next[i][i] - A_k[i][i])
            for i in range(n)
        )
...
def simple_iteration_method(
    phi: callable,       # φ(x), итерационная функция
    x0: float,           # начальное приближение
    eps: float = 1e-9,   # точность
    max_iterations: int = 1000
